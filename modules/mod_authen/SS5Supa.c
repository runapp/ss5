/* Socks Server 5
 * Copyright (C) 2011 by Raffaele De Lorenzo - <raffaele.delorenzo@libero.it>
...
static int
ss5_create_dh_response_from_BN(int s,
...
static int ss5_create_dh_response_from_BN(int s, BIGNUM *public_key_bn)
{
    unsigned char *public_key = NULL;
    uint32_t len_key = 0;
    int ret = 0;

    len_key = BN_num_bytes(public_key_bn);
    ...
    BN_bn2bin(public_key_bn, public_key);

    ret = ss5_create_dh_response(s, public_key, len_key);
    free(public_key);
...
static unsigned char *ss5_secure_dh_compute_key(int s, struct ss5_dh_req *pippo)
{
    ...
    DH *ss = DH_new();
    BIGNUM *a, *p, *g, *public_key_bn, *private_key_bn;
    int len = 1;
   ...
   */
    ...
    do
    {
        DH_set0_key(ss, NULL, BN_secure_new());
        ...
        DH_get0_key(ss, &public_key_bn, &private_key_bn);
    } while (BN_cmp(public_key_bn, BN_value_one()) < 0);

    if (ss5_create_dh_response_from_BN(s, public_key_bn))
    {
        if (VERBOSE())
        {
